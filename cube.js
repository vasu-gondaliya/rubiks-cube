// Array of main colors for each face of the cube (in RGB)
let mainColor = [
  "rgb(255, 0, 0)",      // Red
...
// Defines the stickers affected by turning each face
let sideArray = [
  ["u3", "u6", "u9", "f3", "f6", "f9", "d3", "d6", "d9", "b7", "b4", "b1"], // Right face
  ["b3", "b6", "b9", "d7", "d4", "d1", "f7", "f4", "f1", "u7", "u4", "u1"], // Left face
  ["f1", "f2", "f3", "r1", "r2", "r3", "b1", "b2", "b3", "l1", "l2", "l3"], // Up face
  ["r9", "r8", "r7", "f9", "f8", "f7", "l9", "l8", "l7", "b9", "b8", "b7"], // Down face
  ["d1", "d2", "d3", "r7", "r4", "r1", "u9", "u8", "u7", "l3", "l6", "l9"], // Front face
  ["u1", "u2", "u3", "r3", "r6", "r9", "d9", "d8", "d7", "l7", "l4", "l1"], // Back face
];

// Defines the positions for rotating the center face itself
let faceArray = ["3", "2", "1", "4", "7", "8", "9", "6"];

// Moves notation
let moves = ["R", "L", "U", "D", "F", "B"];

// Maps direction initials to index (e.g., "r" -> 0)
let direction_index = new Map();
for (let i = 0; i < 6; i++) {
  direction_index.set(direction[i][0], i);
}

// Function to rotate a face and its corresponding side stickers
function turn(index, face) {
  // Rotate center face colors
  let faceColorArray = [];
  for (let i = 0; i < 8; i++) {
  ...
  // Rotate side stickers affected by the face turn
  let sideColorArray = [];
  for (let i = 0; i < 12; i++) {
...
// Matrix used to translate face moves depending on the cube's current orientation
let translationMatrix = [
  [0, 'r', 'b', 'l', 'f', 'r', 'u', 'l', 'd', 'f', 'u', 'b', 'd', 'r', 'd', 'l', 'u', 'f', 'd', 'b', 'u', 'r', 'f', 'l', 'b'],
...
// Function for normal face turns based on current orientation
function faceTurn(key) {
  let final_move = translationMatrix[direction_index.get(key)][currentState];
  turn(direction_index.get(final_move), final_move);
}

// Function for inverse (counter-clockwise) face turns
...
// Function to generate a random scramble sequence
function generate() {
  resetColor();
  let sequence = "";
  let sequenceArray = [];

  // Generate 30 random face moves
  for (let i = 0; i < 30; i++) {
  ...
  // Compress repeated moves (e.g., R R -> R2)
  for (let i = 0; i < 30; i++) {
    let count = 1;
    ...
    switch (count) {
      case 1:
        ...
        break;
      case 2:
        sequence += moves[sequenceArray[i]] + "2";
        break;
      case 3:
        sequence += moves[sequenceArray[i]] + "'";
        break;
...
// Array to handle cube rotations (camera view) in 3D
let stateArray = [
  [0, 0, 0, 0],
  [2, 5, 4, 13],
  [3, 19, 1, 11],
...
// Function to rotate the cube view with arrow keys
...
// Starts random cube movements (animation)
...
// Button click listeners for face turns and cube rotations
...
// Map cube rotation keys to arrow keycodes
